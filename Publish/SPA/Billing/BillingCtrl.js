app.controller("BillingCtrl", function ($scope, $http, blockUI) {
    ...
    $scope.post = function (trnType) {
        ...
        $http({
            url: '/Billing/GetDynamic?where=' + where + '&orderBy=BillingName',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
                alertify.log($scope.entity.BillingName + ' already exists!', 'already', '5000');
                $('#txtFocus').focus();
            } else {
                if (trnType === 'save') {
                    trnType = $scope.entity.BillingId === 0 ? "INSERT" : "UPDATE";
