app.controller("PaymentMethodCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function clear() {
        $scope.entity = { FeesTypeId: 0, IsActive: true };
        $("#txtFocus").focus();
    };
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/FeesType/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        var where = "FeesName = '" + $scope.entity.FeesName + "'";
        if ($scope.entity.FeesTypeId > 0)
            where += " AND FeesTypeId <> " + $scope.entity.FeesTypeId;
        ...
        $http({
            ...
            if (data.length > 0) {
                alertify.log($scope.entity.FeesName + ' already exists!', 'already', '5000');
                $('#txtFocus').focus();
            } else {
                if (trnType === 'save') {
                    trnType = $scope.entity.FeesTypeId === 0 ? "INSERT" : "UPDATE";
