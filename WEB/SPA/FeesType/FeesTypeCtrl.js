app.controller("FeesTypeCtrl", function ($scope, $cookieStore, $window, $location, $http, blockUI) {
    ...
    function clear() {
        $scope.entity = { FeesId: 0, IsActive: true };
        $("#txtFocus").focus();
    };
    ...
    $scope.post = function (trnType) {
        ...
        if ($scope.entity.FeesId > 0)
            where += " AND FeesId <> " + $scope.entity.FeesId;
        ...
        $http({
            ...
            if (data.length > 0) {
                ...
                if (trnType === 'save') {
                    trnType = $scope.entity.FeesId === 0 ? "INSERT" : "UPDATE";
